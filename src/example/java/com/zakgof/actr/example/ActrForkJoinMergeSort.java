...
public class ActrForkJoinMergeSort {
    ...
    private static class MasterActor {
        ...
        public void start(int[] array) {
            IActorRef<Sorter> sorter = Actr.system().actorOf(Sorter::new, "c");
    ...
    private static class Sorter {
        ...
        public void run(int[] array, Consumer<int[]> callback) {
            if (array.length == 1) {
                ...
                Actr.system().<Integer, Sorter> forkBuilder(Arrays.asList(0, 1)).constructor(id -> new Sorter()).<int[]> ask((id, sorter, cb) -> sorter.run(id == 0 ? left : right, cb), map -> join(map, callback));
            }
        }
